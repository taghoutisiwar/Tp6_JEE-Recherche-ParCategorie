...
public class ControleurServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (path.equals("/index.do")) {
		...
		} else if (path.equals("/produit_cat.do")) {
			CategorieModele model1 = new CategorieModele();
			List<Categorie> cats = metier1.getAllCategories();
			model1.setCategories(cats);

			request.setAttribute("model1", model1);

			request.getRequestDispatcher("produit_cat.jsp").forward(request, response);
			...
			String sIdCat = request.getParameter("cat");
			Long idCat = Long.parseLong(sIdCat);

			List<Produit> prods = metier.produitsParIdCat(idCat);
			model.setProduits(prods);

			request.setAttribute("idC", idCat);
			request.setAttribute("model", model);
...

		{
			...
			response.sendRedirect("chercher.do?motCle=" + model.getMotCle());
		} else {
